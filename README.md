...
### 1. Fill in needs, then get personalized recommendations / 填写需求，获得个性化推荐
...
### 2. No result in database, then search online / 库内无结果，联网检索补全信息
...
### 3. Interested models are added to Favorites / 感兴趣车型加入喜欢列表
...
### 4. Sales lead collection with Lark webhook notification / 销售线索收集与飞书 Webhook 通知
...
- `app/` - Next.js app routes and global styles / Next.js 路由与全局样式
- `components/` - Reusable UI and page sections / 可复用 UI 组件与页面模块
- `public/` - Static image assets / 静态图片资源
- `lib/` - Shared utilities / 公共工具函数
